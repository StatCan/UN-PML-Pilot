...
@click.option('-r', '--number_of_rounds', prompt=False,
              default=25, show_default=True, type=int)
@click.option('-e', '--number_of_epochs', prompt=False,
...
def run_server(servername: str, min_fit_clients: int,
    ...
    logmessage.update({"Rounds": number_of_rounds, "Epochs": number_of_epochs, "Number of Clients": min_available_clients, "Round accuracies": {}, "Round losses": {}})
    global Model_dict
    ...
    try:
        ...
        while True:
            ...
            logmessage.update({"Time": end_of_training-start_training})
            server_logger.info(json.dumps(logmessage))
            print()
            break
...
def get_eval_fn(model: object,
    ...
    def evaluate(weights: fl.common.Weights) -> Tuple[float, float]:
        ...
        logmessage["Round accuracies"].update({round_counter: accuracy})
        logmessage["Round losses"].update({round_counter: test_loss})
        round_counter += 1
        if round_counter == NUMBER_OF_ROUNDS + 1:
            final_accuracy = accuracy
            final_loss = test_loss
            logmessage.update({'Final Accuracy': final_accuracy, "Final loss": final_loss})
    # save the model
